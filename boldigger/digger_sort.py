...
def get_full_data(path):
    ...
    for level in levels:
        full_data[level] = np.where(
            full_data[level].str.contains("[{}]".format(specials)),
            np.nan,
            full_data[level],
        )
    ...
    full_data["Species"] = full_data["Species"].replace("No Match", "NoMatch")
    full_data["Species"] = full_data["Species"].str.split(" ").str[0]
    full_data["Species"] = full_data["Species"].replace("NoMatch", "No Match")
...
def add_flags(jamp_hit, full_data_df):
    ...
    out = pd.DataFrame(
        {"count": out_df.groupby(level, sort=False).size()}
    ).reset_index()
    if len(out) > 1:
        flags[1] = True
...
def save_results(xlsx_path, dataframe):
    with pd.ExcelWriter(
        Path(xlsx_path), mode="a", engine="openpyxl", if_sheet_exists="replace"
    ) as writer:
    ...
    while True:
        ...
        if not ran:
            ...
            if otu_dfs != "Wrong file":
                window["out"].print(
                    "%s: Filtering data for JAMP hits."
                ...
                if get_full_data(xlsx_path) != "No metadata":
                    ...
                    window["out"].print(
                        "%s: No additional data found. Run again after the additional data download. Close to continue."
                        % datetime.datetime.now().strftime("%H:%M:%S")
                    )
                    window.Refresh()
                    ran = True
                ...
                window["out"].print(
                    "%s: Wrong file format. Close to continue."
                    % datetime.datetime.now().strftime("%H:%M:%S")
                )
                window.Refresh()
                ran = True
