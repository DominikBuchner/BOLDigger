...
def main():
    ...
    layout = [
            ...
            sg.Frame(
                layout=[
                    ...
                    [
                        ...
                        sg.InputText(
                            size=(40, 1), do_not_clear=True, key="output_folder"
                        ),
                        sg.FolderBrowse(),
                    ],
                ],
                ...
                layout=[
                    [
                        ...
                        sg.CB(
                            "Remember me",
                            key="rem_pw",
                            tooltip="This will save your userdata\non your computer without encryption!",
                        ),
        ...
        [
            sg.Frame(
                layout=[
                    [
                        sg.Text("Select a database"),
                        sg.Radio("COI", "database", key="coi", default=True),
                        sg.Radio("ITS", "database", key="its"),
                        sg.Radio("rbcL & matK", "database", key="rbcl"),
                        sg.Spin(
                    ...
                    [
                        sg.Text("Select a fasta file"),
                        sg.InputText(size=(40, 1), do_not_clear=True, key="fasta_path"),
                        sg.FileBrowse(),
                        sg.Button("Check fasta", key="fasta_check"),
                        sg.Button("Run", key="id_eng", button_color=("white", "red")),
                    ],
                ],
        ...
        [
            sg.Frame(
                layout=[
                    [
                        sg.Text("Select a BOLDResults file"),
                        sg.InputText(size=(40, 1), do_not_clear=True, key="resultpath"),
                        sg.FileBrowse(),
                        sg.Button("Run", key="add_data", button_color=("white", "red")),
                    ]
                ],
        ...
        [
            sg.Frame(
                layout=[
                    [
                        sg.Text(
                            "Select a method to determine the top hit (BOLDigger method requires additional data)"
                        )
                    ],
                    [
                        sg.Radio(
                            "Use first hit", "sort_method", key="firsthit", default=True
                        ),
                        sg.Radio("JAMP Pipeline", "sort_method", key="jamp"),
                        sg.Radio("BOLDigger", "sort_method", key="digger"),
                        sg.Button("Run", key="tophit", button_color=("white", "red")),
                    ],
                ],
            ...
            sg.Frame(
                layout=[
                    [
                        sg.Text("Select a BOLDResults file"),
                        sg.InputText(
                            size=(40, 1), do_not_clear=True, key="xlsx_path_api"
                        ),
                        sg.FileBrowse(),
                    ],
                    [
                        sg.Text("Select corresponding fasta file"),
                        sg.InputText(
                            size=(40, 1), do_not_clear=True, key="fasta_path_api"
                        ),
                        sg.FileBrowse(),
                        sg.Button(
                            "Run", key="query_api", button_color=("white", "red")
                        ),
                    ],
                ],
        ...
        [
            ...
            sg.Button(image_data=github, key="github", pad=((640, 0), 0)),
        ],
    ]
    ...
    while True:
        ...
        if version != most_recent_version and update_check:
            update_check = False
            sg.popup(
                "A new version of BOLDigger is available.\nPlease close the application and update.",
                title="Update",
            )
        ...
        if event == "login_check":
            session = login.login(
                values["username"], values["password"], values["rem_pw"]
            )

        ## fasta check code
        if event == "fasta_check" and (
            values["coi"] or values["its"] or values["rbcl"]
        ):
